    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for s in raw_splits:
            ...
            if len(s) > self.chunk_size:
                splits.extend(self._split(s, next_seps))
            else:
                splits.append(s)

        # Merge pieces back up to chunk_size
        chunks: list[str] = []
        current_chunk: list[str] = []
        current_len = 0

        for piece in splits:
            additional_len = len(piece) + (len(sep) if current_chunk else 0)
            if current_chunk and (current_len + additional_len > self.chunk_size):
